...
void sptCorFull(int n, int p, double *coords_sp, double *coords_tm, double *theta, std::string &corfn, double *C){
  ...
  for(i = 0; i < n; i++){
    for(j = i; j < n; j++){
      ...
      tm_dist = fabs(coords_tm[i] - coords_tm[j]);

      // evaluate correlation kernel
      if(corfn == "gneiting-decay"){
        C[i * n + j] = gneiting_spt_decay(sp_dist, tm_dist, theta[0], theta[1]);
        C[j * n + i] = C[i * n + j];
      }else if(corfn == "matern-exponential"){

        if(sp_dist * theta[0] > 0.0){
          C[i * n + j] = findMatern(sp_dist, theta[0], theta[1]);
        }else{
          C[i * n + j] = 1.0;
        }

        C[i * n + j] *= exp(- theta[2] * tm_dist);
        C[j * n + i] = C[i * n + j];
...
// Create full spatial-temporal correlation matrix for change of support (COS) problem
void sptCorCOSFull(int n, int p, double *coords_sp, double *coords_tm, double *theta, std::string &corfn, double *C){
  int i, j, k;
  double sp_dist;
  double sp_component = 0.0, tm_component = 0.0;
  double t_a = 0.0, t_b = 0.0, t_c = 0.0, t_d = 0.0;
  ...
  for(i = 0; i < n; i++){
    for(j = i; j < n; j++){
      ...
      // find property of temporal blocks
      t_a = coords_tm[i];
      t_b = coords_tm[n + i];
      t_c = coords_tm[j];
      t_d = coords_tm[n + j];
      ...
      // evaluate correlation kernel
      if(corfn == "matern-exponential"){

        if(sp_dist * theta[0] > 0.0){
          sp_component = findMatern(sp_dist, theta[0], theta[1]);
        }else{
          sp_component = 1.0;
        }

        tm_component = findExpCOS(t_a, t_b, t_c, t_d, theta[2]);

        C[i * n + j] = sp_component * tm_component;
        C[j * n + i] = C[i * n + j];
...
// Create nxn' cross-correlation spatial-temporal matrix under COS problem: type 1
// type 1: from spaially point-referenced, temporally aggregated to spatially point-referenced, temporally point-referenced
void sptCorCrossCOS1(int n, int n_prime, int p, double *coords_sp, double *coords_tm, double *coords_sp_prime, double *coords_tm_prime, double *theta, std::string &corfn, double *C){
  int i, j, k;
  double sp_dist;
  double sp_component = 0.0, tm_component = 0.0;
  double t_a = 0.0, t_b = 0.0, t0 = 0.0;
  ...
  for(i = 0; i < n; i++){
    for(j = 0; j < n_prime; j++){
      ...
      if(corfn == "matern-exponential"){
        ...
        tm_component = findExpCOS1(t_a, t_b, t0, theta[2]);

        C[j * n + i] = sp_component * tm_component;
...
// Create nxn' cross-correlation spatial-temporal matrix under COS problem: type 1
// type 2: from spaially point-referenced, temporally aggregated to spatial-temporal blocks
void sptCorCrossCOS2(int n, int n_prime, int p, double *coords_sp, double *coords_tm, MatrixList& coords_sp_list, int nMC, int *polyIDmap, double *coords_tm_prime, double *theta, std::string &corfn, double *C){
  int i = 0, j = 0, k = 0, s = 0, polyID = 0;
  double sp_dist;
  double sp_component = 0.0, tm_component = 0.0;
  double t_a = 0.0, t_b = 0.0, t_c = 0.0, t_d = 0.0;
  ...
  for(i = 0; i < n; i++){
    for(j = 0; j < n_prime; j++){
      ...
      t_a = coords_tm[i];
      t_b = coords_tm[n + i];
      t_c = coords_tm_prime[j];
      t_d = coords_tm_prime[n_prime + j];

      // find polygon ID
      polyID = polyIDmap[j] - 1;
      ...
      if(corfn == "matern-exponential"){
        ...
        for(s = 0; s < nMC; s++){
          sp_dist = 0.0;
          for(k = 0; k < p; k++){
            sp_dist += pow(coords_sp[k * n + i] - coords_sp_list.at(polyID, s + nMC, k), 2);
          }
          sp_dist = sqrt(sp_dist);
          if(sp_dist * theta[0] > 0.0){
            sp_component += findMatern(sp_dist, theta[0], theta[1]);
...
// Create full spatial-temporal correlation matrix for change of support (COS) problem
// under COS problem: type 2: covariance matrix for spatial-temporal blocks
void sptCorCOSFull2(int n, int p, MatrixList& coords_sp_list, int nMC, int *polyIDmap, double *coords_tm, double *theta, std::string &corfn, double *C){
  int i = 0, j = 0, k = 0, s = 0, polyID_i = 0, polyID_j = 0;
  double sp_dist;
  double sp_component = 0.0, tm_component = 0.0;
  double t_a = 0.0, t_b = 0.0, t_c = 0.0, t_d = 0.0;
  ...
  for(i = 0; i < n; i++){
    for(j = i; j < n; j++){
      ...
      // find polygon IDs for (i,j)-th entry
      polyID_i = polyIDmap[i] - 1;
      polyID_j = polyIDmap[j] - 1;
      ...
      if(corfn == "matern-exponential"){
        ...
        for(s = 0; s < nMC; s++){
          sp_dist = 0.0;
          for(k = 0; k < p; k++){
            sp_dist += pow(coords_sp_list.at(polyID_i, s, k) - coords_sp_list.at(polyID_j, s + nMC, k), 2);
          }
          sp_dist = sqrt(sp_dist);
          if(sp_dist * theta[0] > 0.0){
            sp_component += findMatern(sp_dist, theta[0], theta[1]);
...
// Function to evaluate spatial component of the spatial-temporal correlation function
double findMatern(double dist, double decay, double nu){

  return pow(dist * decay, nu) / (pow(2, nu - 1) * gammafn(nu)) * bessel_k(dist * decay, nu, 1.0);

}

// Function to evaluate temporal component of the spatial-temporal correlation function
// under change of support (COS) problem
double findExpCOS(double a, double b, double c, double d, double phi){

  double tmp = 0.0;
  double oneByphiSq = 1.0 / (phi * phi);
  double twoByphiSq = 2.0 / (phi * phi);

  // find property of temporal blocks
  std::string tm_property = find_tm_property(a, b, c, d);

  if(tm_property == "A1"){
    tmp = oneByphiSq * (intExp(a, d, phi) + intExp(b, c, phi) - intExp(a, c, phi) - intExp(b, d, phi));
  }else if(tm_property == "A2"){
    tmp = oneByphiSq * (intExp(c, b, phi) + intExp(d, a, phi) - intExp(c, a, phi) - intExp(d, b, phi));
  }else if(tm_property == "B"){
    tmp = twoByphiSq * ((phi * (b - a)) + intExp(a, b, phi) - 1);
  }else if(tm_property == "C1"){
    tmp = oneByphiSq * ((2 * phi * (b - c)) + intExp(a, d, phi) + intExp(c, b, phi) - intExp(a, c, phi) - intExp(b, d, phi));
  }else if(tm_property == "C2"){
    tmp = oneByphiSq * ((2 * phi * (d - a)) + intExp(c, b, phi) + intExp(a, d, phi) - intExp(c, a, phi) - intExp(d, b, phi));
  }else if(tm_property == "D1"){
    tmp = oneByphiSq * ((2 * phi * (b - a)) + intExp(a, d, phi) + intExp(c, b, phi) - intExp(c, a, phi) - intExp(b, d, phi));
  }else if(tm_property == "D2"){
    // Write the above case for "D1" by swapping a with c and b with d
    tmp = oneByphiSq * ((2 * phi * (d - c)) + intExp(c, b, phi) + intExp(a, d, phi) - intExp(a, c, phi) - intExp(d, b, phi));
  }else{
    perror("c++ error: tm_property is not correctly specified");
  }

  // scale by abs(b-a) and abs(d-c)
  tmp = tmp / (fabs(b - a) * fabs(d - c));

  // return the temporal component of the correlation function
  return tmp;
...
// Function to evaluate temporal component of the spatial-temporal correlation function
// under change of support (COS) problem
double findExpCOS1(double a, double b, double t, double phi){

  double tmp = 0.0;
  double oneByphi = 1.0 / phi;

  if(a > b){
    perror("c++ error: findExpCOS1: a must be less than b");
  }

  if(t < a){
    tmp = oneByphi * (intExp(t, a, phi) - intExp(t, b, phi));
  }else if(t > b){
    tmp = oneByphi * (intExp(b, t, phi) - intExp(a, t, phi));
  }else if(t > a && t < b){
    tmp = oneByphi * (2.0 - intExp(a, t, phi) - intExp(t, b, phi));
  }else if(t == a){
    tmp = oneByphi * (2.0 - intExp(t, b, phi));
  }else if(t == b){
    tmp = oneByphi * (2.0 - intExp(a, t, phi));
  }else{
    perror("c++ error: findExpCOS1: t is not in the range of [a, b]");
  }
...
// Function to evaluate property of temporal blocks - helper function for findExpCOS
std::string find_tm_property(double a, double b, double c, double d){

  std::string label = "Unknown";

  // a: start of first block, b: end of first block
  // c: start of second block, d: end of second block

  // A1. a < b < c < d
  if(a < b && b < c && c < d) label = "A1";
  // A1. a < b = c < d
  else if(a < b && b == c && c < d) label = "A1";
  // A2. c < d < a < b
  else if(c < d && d < a && a < b) label = "A2";
  // A2. c < d = a < b
  else if(c < d && d == a && a < b) label = "A2";
  // B. a = c, b = d
  else if(a == c && b == d) label = "B";
  // C1. a < c < b < d
  else if(a < c && c < b && b < d) label = "C1";
  // C1. a = c < b < d
  else if(a == c && c < b && b < d) label = "C1";
  // C1. a < c < b = d
  else if(a < c && c < b && b == d) label = "C1";
  // C2. c < a < d < b
  else if(c < a && a < d && d < b) label = "C2";
  // C2. c = a < d < b
  else if(c == a && a < d && d < b) label = "C2";
  // C2. c < a < d = b
  else if(c < a && a < d && d == b) label = "C2";
  // D1. c < a < b < d
  else if(c < a && a < b && b < d) label = "D1";
  // D1. c = a < b < d
  else if(c == a && a < b && b < d) label = "D1";
  // D1. c < a < b = d
  else if(c < a && a < b && b == d) label = "D1";
  // D2. a < c < d < b
  else if(a < c && c < d && d < b) label = "D2";
  // D2. a = c < d < b
  else if(a == c && c < d && d < b) label = "D2";
  // D2. a < c < d = b
  else if(a < c && c < d && d == b) label = "D2";
  else perror("c++ error: temporal block indices are not in order");
...
// Helper function find integration of the exponential kernel over 1-D intervals
// returns exp(- decay * (b - a))
double intExp(double a, double b, double decay){
  ...
  if(a < b){
    ...
    perror("c++ error: exp_int function: a must be less than b");
  }
...
// Constructor and definition of methods - this is used for COS prediction
MatrixList::MatrixList(SEXP mat_list){
    if(Rf_isInteger(mat_list) || Rf_isReal(mat_list)){
      // Scalar input, possibly 0 to indicate "no matrix list"
      if(Rf_length(mat_list) == 1 && REAL(mat_list)[0] == 0){
        n_matrices = 0;
        nrow = 0;
        ncol = 0;
        return;
      }else{
        throw std::runtime_error("Unexpected numeric input; expected 0 or list of matrices.");
      }
    }

    if(!Rf_isNewList(mat_list)){
      throw std::runtime_error("Input must be a list of matrices or 0.");
    }

    n_matrices = Rf_length(mat_list);
    if(n_matrices == 0){
      nrow = 0;
      ncol = 0;
      return;
    ...
    for(int i = 0; i < n_matrices; ++i){
      SEXP mat = VECTOR_ELT(mat_list, i);

      if (!Rf_isReal(mat) || !Rf_isMatrix(mat)){
        throw std::runtime_error("Each element must be a numeric matrix.");
      }

      SEXP dims = Rf_getAttrib(mat, R_DimSymbol);
      int this_nrow = INTEGER(dims)[0];
      int this_ncol = INTEGER(dims)[1];

      if(i == 0){
        nrow = this_nrow;
        ncol = this_ncol;
      }else{
        if(this_nrow != nrow || this_ncol != ncol){
          throw std::runtime_error("All matrices must have the same dimensions.");
        }
      }

      double* raw = REAL(mat);
      std::vector<double> flat_matrix(raw, raw + (nrow * ncol));  // column-major
      data.push_back(std::move(flat_matrix));
    }
  }

// Access element (i-th matrix, row r, col c)
double MatrixList::at(int i, int r, int c) const{
    return data[i][r + nrow * c];  // column-major order
}
